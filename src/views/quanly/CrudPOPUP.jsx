...
const CrudPOPUP = () => {
  ...
  const updateStatus = async () => {
    const res = await axios({
      url: "http://localhost:8080/api/popup/getPopupsAfterUpdateStatus",
      method: "GET",
    });
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      if (createorupdate) {
        values.ngay_tao < currentday.toISOString().split("T")[0]
          ? alertCurrent()
          : values.ngay_tao > values.han_su_dung
          ? alertExpriredday()
          : updatePopUp(values);
      } else {
        console.log(currentday.toISOString().split("T")[0]);
        console.log(values.ngay_tao);
        values.ngay_tao < currentday.toISOString().split("T")[0]
          ? alertCurrent()
          : values.ngay_tao > values.han_su_dung
          ? alertExpriredday()
          ...
          <button
            ...
            onClick={() => {
              ...
              for (let i = 0; i < record.sanpham.length; i++) {
                ...
                product.push(record.sanpham[i].san_phamId);
              }
  ...
  const fetchProductHasDiscount = async () => {
    try {
      const res = await axios({
        url: "http://localhost:8080/FindProducthaspopupid",
        method: "GET",
      });
  ...
  const fetchDataHanhDong = async () => {
    try {
      const response = await axios.get(
        "http://localhost:8080/api/popup/FindAllPopUpwithDTO"
  ...
  const getData = async () => {
    try {
      const res = await axios({
        url: "http://localhost:8080/api/popup/getallpopupnotdeleted",
        method: "GET",
      });
      const formattedData = res.data.map((item, index) => ({
        key: index,
        ...
        trangthaixoa: item?.trang_thai_xoa,
        accountid: item?.users?.accountID,
        sanpham: item?.popupchitiet.map((item) => ({
  ...
  useEffect(() => {}, [personName, listtemp, options2]);
            ...
            <form onSubmit={formik.handleSubmit} className="row mt-5">
              ...
              <div className="col-md-6 my-2">
                ...
                <div className="form-floating mt-3 ">
                  <input
                    value={formik.values.han_su_dung}
                    disabled={formik.values.hoat_dong == "On" && beingedit}
                    type="date"
                    onChange={(e) => {
              ...
              <div className="col-md-12 text-center mt-3">
                {selected ? (
                  <button
                    // disabled={
                    //   personName.length > 3 ||
                    //   (formik.values.hoat_dong == "On" && beingedit)
                    // }
                    className="btn btn-outline-warning fw-bold ms-2 mt-2"
                    onClick={() => setcreateorupdate(true)}
                    type="submit"
                    style={{ minWidth: 120 }}
                  >
                    {" "}
                    {personName.length > 3
                      ? "Chỉ có thể chọn tối đa 3 sản phẩm"
                      : "Cập nhật Danh Mục"}
                  </button>
                ) : (
                  <button
                    className="btn btn-outline-primary fw-bold ms-2 mt-2"
                    onClick={() => setcreateorupdate(false)}
                    disabled={personName.length > 3}
                    type="submit"
                    style={{ minWidth: 120 }}
                  >
                    {" "}
